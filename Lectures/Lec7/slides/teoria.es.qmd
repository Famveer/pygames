...
El sistema de audio de Pygame tiene dos componentes principales:
...
if channel is None:
    print("¡No hay Channel libre!")
```

## Channels

Pygame tiene N Channels de mixer (8 por defecto). Cada Sound se reproduce en un Channel a la vez.
...
print("Spatial audio — stereo pan según posición")
print("=" * 45)
print(f"  {'posición x':<14} {'pan':<8} {'vol izq':<12} {'vol der'}")
print("  " + "-" * 44)
...
- `music` hace streaming desde el disco — úsalo para pistas de fondo largas
- `Sound` se carga completamente en RAM — úsalo solo para SFX cortos
- Cada `Sound` se reproduce en un `Channel`; reserva channels para categorías
- Panning estéreo: `channel.set_volume(left, right)`
- Atenuación por distancia: `volume = max(0, 1 - distance / max_dist)`

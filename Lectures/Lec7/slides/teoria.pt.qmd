...
O sistema de áudio do Pygame tem dois componentes principais:
...
O Pygame tem N Channels de mixer (8 por padrão). Cada Sound é reproduzido em um Channel por vez.
...
print("Spatial audio — stereo pan por posição")
print("=" * 45)
print(f"  {'posição x':<14} {'pan':<8} {'vol esq':<12} {'vol dir'}")
print("  " + "-" * 44)
...
- `music` faz streaming a partir do disco — use para faixas de fundo longas
- `Sound` é carregado totalmente na RAM — use apenas para SFX curtos
- Cada `Sound` é reproduzido em um `Channel`; reserve channels para categorias
- Panning estéreo: `channel.set_volume(left, right)`
- Atenuação por distância: `volume = max(0, 1 - distance / max_dist)`

---
title: "🎮 Modules 2, 3 & 4 — Sprites, Collisions and Animations"
subtitle: "Pygame Game Development Course"
author: "Pygame Course"
date: today
lang: en
format:
  html:
...
- **Module 2:** What a Sprite is, the `pygame.sprite.Sprite` class, groups, and transparency
- **Module 3:** `pygame.Rect` in depth, collision types, and collision response
- **Module 4:** Frame-based animation, spritesheets, and the animation state machine
:::

---

# 📦 MODULE 2 — Sprites & Images
...
print("MODULE 2 SUMMARY — Sprites")
print("=" * 55)
...
Run with `python scripts/script_name.py` from the module folder.

| Script | Module | What it demonstrates |
|--------|--------|----------------------|
| `01_sprite_basico.py` | 2 | Sprite class, image, rect, Group |
| `02_sprite_grupos.py` | 2 | Multiple groups, kill(), transparency |
| `03_colisiones_rect.py` | 3 | Visual interactive AABB |
| `04_colisiones_sprites.py` | 3 | spritecollide, groupcollide, response + particle explosion |
| `05_animacion_frames.py` | 4 | Animation timer, generated spritesheet |
| `06_estados_animacion.py` | 4 | Full state machine (idle/walk/jump/attack) |

```bash
pip install pygame
python scripts/06_estados_animacion.py
```
...
## Module 2 — Sprites {.unnumbered}

- `Sprite` = `image` + `rect` + encapsulated logic
- `Group` = collection with automatic `update()` and `draw()`
- `kill()` = remove a sprite from the game entirely
- `convert_alpha()` = PNG with per-pixel transparency

## Module 3 — Collisions {.unnumbered}
...
## Module 4 — Animations {.unnumbered}
...
::: {.callout-note}
**Next module:** Scenes and Game States — menu → gameplay → game over 🎮
:::
